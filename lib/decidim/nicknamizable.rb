  ...
  module Nicknamizable
    ...
    class_methods do
      ...
      def disambiguate(name, scope)
        ...
        2.step do |n|
          return candidate unless nickname_already_exists?(candidate, scope)

          candidate = numbered_variation_of(name, n)
        end
      end

      def nickname_already_exists?(candidate, scope)
        Decidim::UserBaseEntity.where("LOWER(nickname) = ?", candidate.downcase).where(scope).present?
      end
