  ...
  describe CreateParticipatorySpacePrivateUser do
    ...
    context "when everything is ok" do
      ...
      context "when there is no user with the given email" do
        ...
        it "creates a new user and does not send the govbr notification email" do
          expect { subject.call }.not_to have_enqueued_email(Decidim::Govbr::InvitedToPrivateParticipatorySpaceMailer, :notification)
          expect(Decidim::User.last.email).to eq(email)
        end
      end
      ...
      context "when a private user exist" do
        ...
        it "sends a notification e-mail" do
          expect { subject.call }.to have_enqueued_email(Decidim::Govbr::InvitedToPrivateParticipatorySpaceMailer, :notification)
        end
      end
